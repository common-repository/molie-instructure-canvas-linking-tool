			...
			if(wp_verify_nonce($_POST['nonce'], "molie_admin_quiz"))
			{
				...
				foreach($questions as $question){
					...
					if(get_post_meta($quiz_post, "canvasQuizQuestion_" . $question->id, true)==""){
						...
						update_post_meta($question_post, "canvasQuestion_type", $question->question_type, true);
						...
						if($question->question_type=="fill_in_multiple_blanks_question"){
							...
							foreach($question->answers as $answer){
								...
								update_post_meta($question_post, "qa_weight_" . $counter, $answer->weight, true);
								update_post_meta($question_post, "qa_answer_" . $counter, $answer->text, true);
								update_post_meta($question_post, "qa_blankID_" . $counter, $answer->blank_id, true);
								$counter++;
							}
						}
						...
						if($question->question_type=="short_answer_question"){
							...
							foreach($question->answers as $answer){
								...
								update_post_meta($question_post, "qa_weight_" . $counter, $answer->weight, true);
								update_post_meta($question_post, "qa_answer_" . $counter, $answer->text, true);
								update_post_meta($question_post, "qa_correct_feedback_" . $counter, $answer->comments_html, true);
								update_post_meta($question_post, "qa_incorrect_feedback_" . $counter, $question->incorrect_comments_html, true);
								$counter++;
							}
						}
